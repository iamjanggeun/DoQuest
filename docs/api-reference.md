...
재시도 대상이 아닌 일반 `4xx`, 응답 계약 위반, 날짜·시간 파싱 오류는 반복 호출하지 않고 `FAILED`로 전환합니다. 시도 횟수와 최종 오류는 내부 진단용으로만 저장되며 API 응답 스키마에는 노출하지 않습니다. 별도 재시도 엔드포인트를 추가하지 않고 기존 분석 시작 API를 멱등하게 재사용합니다.
