...
| 2026.08.25 | v0.9.0 | 캘린더 기능을 위한 Schedule 도메인 구축 / 복합 인덱스 기반 D-3 큐레이션 최적화: LLM 반복 호출 없이 idx_schedules_member_date 복합 인덱스 쿼리로 마감 3일 이내 데이터를 즉시 필터링 / Two-Phase Commit UX 지원: 비정형 메모 파싱 후 바로 일정을 강제 저장하지 않고, 유저 확인을 거쳐 ScheduleCreateRequest로 영속화할 수 있도록 함 | Janggeun |
